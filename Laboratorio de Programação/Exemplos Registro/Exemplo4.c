...
main(){
	// todo esse c�digo ser� alterado para fun�oes
	...
	do{
		...
		printf("\nInforme sua opcao: ");
		scanf("%d",&opcao);
		getchar(); //limpeza de buffer - procurar op��o melhor pra corrigir isso aqui
		switch(opcao){
			case 0:
				printf("Finalizando");
			...
			case 4:
				...
				scanf("%c",&optionList);
					if(optionList=='a'){
						//para organizar por idade � s� considerar as letras como string e comparar
					}else if(optionList=='b'){			
					printf("Listando por idade crescente: \n");
						for(int i=0;i<size;i++){
							for(int j=0;j<size-1;j++){
								if(pessoa[j].idade>pessoa[j+1].idade){
									maior = pessoa[j];
									pessoa[j]=pessoa[j+1];
									organizaIdade[j]=pessoa[j+1];
									organizaIdade[j+1] = maior;
									pessoa[j+1] = maior;
								}
								organizaIdade[j]=pessoa[j];
							}
							//printf("%s\n",pessoa[i].nome);//
						}
						//printf("\nDepois de organizar por idade: ");
						for(int x=0;x<size;x++){
							printf("->%d %s\n",x, organizaIdade[x].nome);
						}
					}
				break;
